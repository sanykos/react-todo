    ...
    <Grid container style={{ marginTop: '10px' }}>
      <Grid item xs={4}>
        <ButtonGroup variant="contained" size="large" aria-label="large button group">
          <Button onClick={() => handlers.setFilter(VISIBILITY_FILTER.SHOW_ALL)}>All {anotherProps.totalCount} </Button>
          <Button onClick={() => handlers.setFilter(VISIBILITY_FILTER.COMPLETED)}>
            completed {anotherProps.doneCount}
          </Button>
          <Button onClick={() => handlers.setFilter(VISIBILITY_FILTER.ACTIVE)}>active {anotherProps.todoCount} </Button>
        </ButtonGroup>
      </Grid>
      <Grid item xs={8}>
